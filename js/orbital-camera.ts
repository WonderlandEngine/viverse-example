...
export class OrbitalCamera extends Component {
	...
	start(): void {
		this._polar = Math.min(
			this.maxElevation,
			Math.max(this.minElevation, this._polar),
		);
		this._updateCamera();
	}
	...
	getClosestPosition(position: vec3, rotation: quat) {
		...
		if (quat.dot(tempquat, tempquat2) < 0) {
			quat.scale(
				...
				tempquat2,
				-1,
			); /* Negate to ensure shortest path */
		}
	...
	private _updateCamera() {
		...
		tempVec[0] =
			this.radial * Math.sin(azimuthInRadians) * Math.cos(polarInRadians);
		tempVec[1] = this.radial * Math.sin(polarInRadians);
		tempVec[2] =
			this.radial * Math.cos(azimuthInRadians) * Math.cos(polarInRadians);
	...
	private _onMouseScroll = (e: WheelEvent) => {
		...
		this.radial = Math.min(
			this.maxZoom,
			Math.max(this.minZoom, this.radial),
		);
	...
	private _onTouchMove = (e: TouchEvent) => {
		...
		if (e.touches.length === 1) {
			...
			/* Handle pinch zoom */
			const currentPinchDistance = this._getDistanceBetweenTouches(
				e.touches,
			);
			const pinchScale =
